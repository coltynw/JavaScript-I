...
const findCar = (carInventory, id) => {
  for (let i = 0; i < carInventory.length; i++) {
    if (carInventory[i].id===id) {
      return `Car 33 is a ${carInventory[i].car_year} ${carInventory[i].car_make} ${carInventory[i].car_model}`
    }
  }
}
...
// ==== Challenge 4 ====
let carYears = [];
const years = (carInventory) => {
    
    for (let i = 0; i < carInventory.length; i++) {
      carYears.push(carInventory[i].car_year);
      }
      return carYears.toString();
  }
  console.log(years(inventory));
...
for (let i=0; i < carMake.length; i++) {
  ...
  if (carMake[i] === 'Audi') {BMWAndAudi.push(inventory[i])}
}
